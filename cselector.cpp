...
void CSelector_Draw(CSelector *Selector)
{
	...
	screen.stretch_blit(IMGSelector,
        Rect(0,0,IMGSelector->bounds.w, IMGSelector->bounds.h),
        Rect(xoffset + (BoardXOffSet + Selector->CurrentPoint.X * (TileWidth)) *scale,
			yoffset + (BoardYOffSet+ Selector->CurrentPoint.Y * (TileHeight))* scale, 
