...
CPeg* CPeg_Create(const int PlayFieldXin,const int PlayFieldYin)
{
    ...
    Result->AnimPhase = 0; // current frame = 0 (a red dot)
 	...
 	Result->X=BoardXOffSet + PlayFieldXin*TileWidth; //set the screen position (for drawing)
 	Result->Y=BoardYOffSet + PlayFieldYin*TileHeight;
...
void CPeg_SetPosition(CPeg* Peg, const int PlayFieldXin,const int PlayFieldYin)
{
	if ((PlayFieldXin >= 0) && (PlayFieldXin < NrOfCols) && (PlayFieldYin >= 0) && (PlayFieldYin < NrOfRows))
	{
		...
		Peg->X=BoardXOffSet + PlayFieldXin*TileWidth;
		Peg->Y=BoardYOffSet + PlayFieldYin*TileHeight;
	}
}

//workaround blitsim bug
int TmpLowX,TmpLowY;
...
void CPeg_Draw(CPeg* Peg) // drawing
{
	if (Peg->Image)
	{
		...
		DstRect.x = xoffset + Peg->X*scale;
		DstRect.y = yoffset + Peg->Y*scale;
		DstRect.w = TileWidth*scale;
		DstRect.h = TileHeight*scale;
		screen.stretch_blit(Peg->Image, SrcRect, DstRect); 
	}
}
